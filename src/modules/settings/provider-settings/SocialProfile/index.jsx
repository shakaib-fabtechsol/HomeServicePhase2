    ...
    const onSubmit = async (data) => {
        try {
            ...
            Swal.fire({
                icon: 'error',
                title: 'Oops...',
                text: error?.message || 'Something went wrong while connecting your social profile.',
            });
        }
    };
                    ...
                    <div key={index} className="py-5 border-b border-[#E9EAEB]">
                        <div className="flex items-center flex-wrap gap-3 justify-between py-3 px-4 bg-[#FAFAFA] min-h-[60px] rounded-[8px]">
                            ...
                            <div className="ms-auto">
                                {social.link ? (
                                    <Link to="" >
                                        <CiTrash className="text-[24px]" onClick={()=>handleDeleteSocial(social.link, social.name)}/>
                                    </Link>
                                ) : (
                                    <button
                        ...
                        <div className="bg-white rounded-[12px] p-4 max-h-[calc(100dvh-200px)] overflow-y-auto">
                            <form onSubmit={handleSubmit(onSubmit)}>
                                <div className="flex flex-col items-center mt-5 gap-5">
                                    ...
                                    <div>
                                        <input
                                            className={`border w-full p-3 rounded-[8px] focus:outline-none ${errors[selectedSocial.name.toLowerCase().replace(" ", "_")]
                                                    ? "border-red-500"
                                                    : "border-[#D5D7DA]"
                                                }`}
                                            type="text"
                                            {...register(selectedSocial.name.toLowerCase().replace(" ", "_"), {
                                        ...
                                        {errors[selectedSocial.name.toLowerCase().replace(" ", "_")] && (
                                            <p className="text-red-500 text-sm mt-1">
                                                {errors[selectedSocial.name.toLowerCase().replace(" ", "_")].message}
                                            </p>
                                        )}
                                    </div>
